    ...
    if request.method == "GET":
        ...
        if not name:
            ...
            if cate:
                ...
                newCat = Cat(name=request.form['name'],user_id=login_session['user_id'])
                session.add(newCat)
                session.commit()
...
def editItem(cat_id, item_id):
    ...
    if editedItem.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert( 'You are not authorized to edit this restaurant./Please create"+
        " your own restaurant in order to edit.');}/</script><body onload='myFunction()''>"
    if request.method == 'POST':
